...
protocol WishListInteracting {
    func getWishList() -> [BookModel]
}

final class WishListInteractor: WishListInteracting {
    func getWishList() -> [BookModel] {
        ...
        if sqlite3_open(path, &db) == SQLITE_OK {
            let query = "SELECT book_id, book_name, is_read, is_wishlist, author_id, description, image FROM books WHERE author_id = 1"
...
private extension WishListInteractor {
